...
if page == "User Management":
    ...
    if not users:
        ...
        from services.calorie_service import get_all_foods, get_daily_log, save_daily_log, get_all_activities, delete_daily_log
            ...
            df_food = pd.DataFrame([
                {
                    ...
                    "Food": f.food_name,
                    "Group": f.food_group,
                    "Calories": f"{f.calories:.0f}",
                    "Protein": f"{f.protein:.1f}g",
                    "Fat": f"{f.fat:.1f}g",
                    "Carbohydrates": f"{f.carbs:.1f}g",
                    "Portion": f.portion
                } for f in daily_log.foods
            ])
            st.dataframe(df_food, use_container_width=True, height=300)
            ...
            df_act = pd.DataFrame([
                {
                    ...
                    "Motion": a.specific_motion,
                    "Duration": f"{a.duration_minutes:.0f}m",
                    "Burnt": f"{a.calories_burnt:.1f} kcal"
                } for a in daily_log.activities
            ])
